...
export class Lcu {
  ...
  public static async getCsInput(prevCsInput: CsInput, all_info_alt: any, csInProgress: boolean): Promise<CsInput> {
    ...
    try {
      ...
      newCsInput.hiddenSummoners = (blueTeam.map(x => x.hidden).concat([false, false, false, false, false])).slice(0, 5).concat((redTeam.map(x => x.hidden).concat([false, false, false, false, false])).slice(0, 5));
  ...
  private static async riotRequest(query: string): Promise<any> {
    ...
    const resO : any = await Promise.race([
      new Promise<overwolf.web.SendHttpRequestResult>(resolve => overwolf.web.sendHttpRequest(url, overwolf.web.enums.HttpRequestMethods.GET, [
